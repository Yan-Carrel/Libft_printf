...
/*   Updated: 2026/02/24 16:39:34 by yaandria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int    spaces_to_print(infos print_info, int print_len, int zeros_len, int neg);
...
static int    print_right(infos print_info, long nbr, int zeros, int print_len)
{
    ...
    return (print_len + zeros + spaces + neg);
}
...
static int handle_signs(infos print_info, int neg)
{
    ...
    {
        if (print_info.plus == '+' && (print_info.specifier == 'd' || print_info.specifier == 'i'))
        {
            ft_putchar_fd('+', 1);
            count++;
        }
...
static int spaces_to_print(infos print_info, int print_len, int zeros_len, int neg)
{
    int sign_len = (neg || (print_info.plus == '+' && (print_info.specifier == 'd' || print_info.specifier == 'i'))) ? 1 : 0;

    if (print_info.pad == '0' && print_info.precision < 0)
    {
        // zeros will pad width minus sign
        return (print_info.width > (print_len + sign_len)) ? print_info.width - (print_len + sign_len) : 0;
    }
    else
    {
        return (print_info.width > (print_len + zeros_len + sign_len)) ? print_info.width - (print_len + zeros_len + sign_len) : 0;
    }
}
