...
/*   Updated: 2026/02/25 15:31:21 by yaandria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_pointer(infos info, unsigned long nbr)
{
	...
	if (info.precision > 0 && info.precision > digits_len)
	...
	if (info.justification == 'l')
        ...
        print_caseB(nbr, precision_zeros, digits_len, padding);
	if (info.width > total_len)
		return (info.width);
	return (total_len);
}
...
void   print_caseB(unsigned long nbr, int precision_zeros, int digits_len, int padding)
{
    put_padding(' ', padding);
    ft_putstr_fd("0x", 1);
    put_padding('0', precision_zeros);
